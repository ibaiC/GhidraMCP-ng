...
def get_socket_dir() -> Path:
    ...
    if platform.system() == "Windows":
        # Windows: use %TEMP% (matches Java's tmpdir behavior)
        tmpdir = os.environ.get("TEMP", os.environ.get("TMP", ""))
        user = os.environ.get("USERNAME", "unknown")
        if tmpdir:
            return Path(tmpdir) / f"ghidra-mcp-{user}"
        return Path(os.path.expanduser("~")) / ".ghidra-mcp"
...
def _discover_tcp_instance(tcp_url: str) -> dict | None:
    """Probe a TCP endpoint for a running Ghidra instance. Returns info dict or None."""
    info: dict = {"transport": "tcp", "url": tcp_url}
    try:
        # Try UDS-style instance_info first (available if UDS endpoints are mirrored)
        text, status = tcp_request(tcp_url, "GET", "/mcp/instance_info", timeout=5)
        if status == 200:
    ...
    # Fall back to TCP-native endpoints (/project/info + /get_version)
    try:
        text, status = tcp_request(tcp_url, "GET", "/project/info", timeout=5)
        if status == 200:
            data = json.loads(text)
            info["project"] = data.get("project")
            info["open_programs"] = data.get("open_programs", [])
        else:
            # /project/info failed — try bare connection check
            text, status = tcp_request(tcp_url, "GET", "/check_connection", timeout=5)
            if status != 200:
                return None
    except Exception as e:
        logger.debug(f"TCP probe {tcp_url} failed: {e}")
        return None

    try:
        text, status = tcp_request(tcp_url, "GET", "/get_version", timeout=5)
        if status == 200:
            data = json.loads(text)
            info["version"] = data.get("full_version")
    except Exception:
        pass
...
def discover_instances() -> list[dict]:
    """Scan for running Ghidra instances via UDS sockets and TCP probing."""
    instances = []

    # UDS discovery (Linux/macOS, or Windows with UDS enabled)
    ...
    if socket_dir.exists():
        for sock_file in sorted(socket_dir.glob("*.sock")):
            ...
            info: dict = {"socket": str(sock_file), "pid": pid, "transport": "uds"}
            try:
    ...
    # TCP discovery — probe the default TCP endpoint (and GHIDRA_MCP_URL if set)
    # This is essential on Windows where UDS is disabled by default.
    if not instances:
        tcp_urls = []
        env_url = os.getenv("GHIDRA_MCP_URL")
        if env_url:
            tcp_urls.append(env_url)
        if DEFAULT_TCP_URL not in tcp_urls:
            tcp_urls.append(DEFAULT_TCP_URL)
        for url in tcp_urls:
            info = _discover_tcp_instance(url)
            if info:
                instances.append(info)
...
def _try_reconnect() -> bool:
    ...
    def _reconnect_to(inst: dict) -> bool:
        global _active_socket, _active_tcp, _transport_mode
        if inst.get("transport") == "tcp":
            _active_tcp = inst["url"]
            _active_socket = None
            _transport_mode = "tcp"
        else:
        ...
        try:
            ...
            logger.info(f"Reconnected to project '{inst.get('project')}' via {_transport_mode}")
            return True
        except Exception as e:
...
def list_instances() -> str:
    """
    List all running Ghidra instances discovered via UDS sockets or TCP probing.

    Returns JSON with each instance's project name, PID, open programs, and connection path.
...
async def connect_instance(project: str, ctx: Context | None = None) -> str:
    ...
    # Match by project name across all discovered instances (UDS and TCP)
    if instances:
        match = None
        for inst in instances:
        ...
        if not match:
            for inst in instances:
                if project.lower() in inst.get("project", "").lower():
                    ...
                    break
        if match:
            if match.get("transport") == "tcp":
                _active_tcp = match["url"]
                _active_socket = None
                _transport_mode = "tcp"
            else:
            ...
            try:
                ...
                if _transport_mode == "uds":
                    result["socket"] = match["socket"]
                else:
                    result["url"] = match["url"]
                return json.dumps(result)
            except Exception as e:
                return json.dumps({"error": f"Schema fetch failed: {e}",
                                   "transport": _transport_mode})

    # Try TCP fallback (no instances discovered, or none matched)
...
def _auto_connect():
    ...
    # discover_instances() now probes both UDS and TCP
    instances = discover_instances()
    if len(instances) == 1:
        inst = instances[0]
        if inst.get("transport") == "tcp":
        ...
        logger.info(f"Auto-connecting via {_transport_mode} to {_connected_project or 'unknown'}")
        try:
            count = _fetch_and_register_schema()
            logger.info(f"Auto-registered {count} tools from {_connected_project or 'unknown'}")
            return
        except Exception as e:
            logger.warning(f"Auto-connect schema fetch failed ({_transport_mode}): {e}")
            _active_socket = None
            _active_tcp = None
            _transport_mode = "none"
    ...
    # Last-resort TCP fallback if discover_instances() found nothing
    if not instances:
        tcp_url = os.getenv("GHIDRA_MCP_URL", DEFAULT_TCP_URL)
        try:
            _active_tcp = tcp_url
